...
function get_migration_from_class_section(class_name_id)
{
     if(!class_name_id)
     {
        var str = '<option value="" >Select One</option>';
        $('#from_section_id').html(str);
        return false;
     }
     
     
     $.ajax({ 
          url: baseUrl+'create/ajax_migration_class_section',        
          data:
          {
                'class_name_id':class_name_id
          },
          ...
          success: function(data)
          {
              ...
              if(result == 'success')
              {
                 $('#from_section_id').html(section_id);   
              }
              else if(result == 'fail')
              {
...
function get_migration_to_class_section(class_name_id)
{
     if(!class_name_id)
     {
        var str = '<option value="" >Select One</option>';
        $('#to_section_id').html(str);
        return false;
     }
...
function get_student_list_class_sec(section_id)
{
    var class_name_id = $('#class_name_id').val();
    if(section_id)
    {
       if(!class_name_id && !section_id)
        {
            $('#student_list_con').html(''); 
            return false;
        }
        $.ajax({ 
        url: baseUrl+'create/student_list_ajax_migrated',        
        data:
        {
                'class_name_id':class_name_id,
                'section_id':section_id
