...
class Create extends CI_Controller {
    ...
    function edit_teacher($teacher_id)
    {
        if(isPostBack())
        {
             ...
             $this->Admin_model->edit_table_data($data,'scl_teacher','teacher_id',$teacher_id);
             message('Teacher has been updated successfully');

        }

        $data['class'] = $this->Admin_model->get_distinct_class($_SESSION['school_id']);
        $data['tinfo'] = $this->Admin_model->get_teacher_detail($teacher_id,$_SESSION['school_id']);
        $data['mainContent'] = $this->load->view('admin/teacher/edit_teacher', $data, true);
        $this->load->view('admin/template', $data);
    }
    /*NEW CODE ENDS HERE*/
