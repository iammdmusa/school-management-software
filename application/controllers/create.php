...
class Create extends CI_Controller {
    ...
    function send_greeting_sms($greeting_id)
    {
         ...
         if($smsStudentInfo)
         {
            ...
            $username=  'tareq@xenontech.net';
            $password=  '509539tareq';
    ...
    function student()
    {
        if(isPostBack())
        {
            ...
            $data['display_id']               = $_POST['display_id']; 
            $data['name']                     = $_POST['name']; 
            ...
            $exists = $this->custom_model->checkDisplayID($data['display_id']);
            if($exists) {
                $data['message'] = "Student ID: " . $data['display_id'] . " exists";
            } else {
    ...
    function custom_message()
    {
       if(isPostBack())
       {
             if(strlen($_POST['message']) > 160 ){ die('Maximum 160 characters allowed'); }
    ...
    function resultlist()
    {
        //$this->load->model('custom_model');
        $data['class'] = $this->Admin_model->get_distinct_class($_SESSION['school_id']);
        //$data['exams'] = $this->custom_model->getExams($_SESSION['school_id']);
    ...
    function edit_student($student_id)
    {
        ...
        {
            $this->load->model('custom_model');
             $new_display_id = $_POST['display_id'];
             $old_display_id = $_POST['old_display_id'];
            
             $data['display_id']               = $new_display_id; 
             $data['name']                     = $_POST['name']; 
             $data['father_name']              = $_POST['father_name']; 
             $data['mother_name']              = $_POST['mother_name']; 
             $data['gurdian_name']             = $_POST['gurdian_name']; 
            ...
            if($old_display_id !== $new_display_id) {
                $exists = $this->custom_model->checkDisplayID($new_display_id);
                if($exists) {
                    $data['message'] = "Student ID: " . $new_display_id . " exists";
                } else {
                    $this->Admin_model->edit_table_data($data,'scl_student','student_id',$student_id); 
                    message('Student has been updated successfully');
                    redirect("create/edit_student/$student_id");
                }
            } else {
                $this->Admin_model->edit_table_data($data,'scl_student','student_id',$student_id); 
                message('Student has been updated successfully');
